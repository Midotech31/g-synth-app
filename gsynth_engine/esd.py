"""Extended Sequence Design (ESD) — long constructs from annealed oligo pairs.

The workflow formalized by G-Synth:
...
@dataclass
class ESDResult:
    """Everything needed to order oligos and execute an ESD assembly."""
...
def design_extended_sequence(
    ...
    *,
    ...
    """Apply Extended Sequence Design to one insert.
    ...
    Args:
        ...
        overhang_length: 4–8 nt, per the ESD workflow.
        search_window: how far a junction may move from its ideal position
            to find a usable overhang.

    Returns:
        An :class:`ESDResult`. Call ``verify()`` before ordering — it
