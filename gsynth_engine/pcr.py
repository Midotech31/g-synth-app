...
MIN_ANNEAL: int = 18
MAX_ANNEAL: int = 30
MIN_MANUAL_ANNEAL: int = 15
MAX_MANUAL_ANNEAL: int = 60
...
class PcrResult:
    ...
    alternative_pairs: list[dict[str, str]] = field(default_factory=list)
...
def _pick_anneal(
    template: str, *, start: int, direction: int, fixed_length: int | None = None,
) -> tuple[str, int, int]:
    ...
    if fixed_length is not None:
        lengths = range(fixed_length, fixed_length + 1)
    else:
        lengths = range(MIN_ANNEAL, MAX_ANNEAL + 1)
    ...
    if best is None:
        raise SequenceError(
            ...
            f"{fixed_length or MIN_ANNEAL} bases are needed at this end of "
            f"the region you want to amplify."
        )
...
def design_pcr(
    ...
    *,
    ...
    forward_anneal_length: int | None = None,
    reverse_anneal_length: int | None = None,
    name: str = "product",
    ...
    Args:
        ...
        forward_anneal_length / reverse_anneal_length: set both for a manual
            footprint. Omit both to let G-Synth choose 18–30 nt toward 60 °C.
    ...
    manual_lengths = (forward_anneal_length, reverse_anneal_length)
    if (forward_anneal_length is None) != (reverse_anneal_length is None):
        raise SequenceError(
            "Set both manual primer lengths, or leave both automatic."
        )
    for label, length in zip(("forward", "reverse"), manual_lengths, strict=True):
        if length is not None and not MIN_MANUAL_ANNEAL <= length <= MAX_MANUAL_ANNEAL:
            raise SequenceError(
                f"The manual {label} annealing length must be between "
                f"{MIN_MANUAL_ANNEAL} and {MAX_MANUAL_ANNEAL} bases."
            )
    ...
    fwd_anneal, fwd_lo, fwd_hi = _pick_anneal(
        ...
        fixed_length=forward_anneal_length,
    )
    rev_anneal, rev_lo, rev_hi = _pick_anneal(
        working, start=stop, direction=-1,
        fixed_length=reverse_anneal_length,
    )
    ...
    if fwd_hi > rev_lo:
        raise SequenceError(
            ...
            f"the selected primer footprints overlap. Shorten the manual "
            f"lengths or amplify a longer region."
        )
    ...
    if forward_anneal_length is not None:
        for label, length in zip(("forward", "reverse"), manual_lengths, strict=True):
            if length is not None and not MIN_ANNEAL <= length <= MAX_ANNEAL:
                warnings.append(
                    f"The manual {label} footprint is {length} nt. G-Synth's "
                    f"automatic specificity range is {MIN_ANNEAL}–{MAX_ANNEAL} nt; "
                    f"review off-target binding before ordering."
                )
    ...
    if (
        suggest_alternatives and problems
        and left_enzyme is not None and right_enzyme is not None
    ):
        result.alternative_pairs = _suggest_restriction_pairs(
            working,
            target_start=target_start,
            target_end=stop,
            excluded=(left_enzyme, right_enzyme),
            clamp=clamp,
            keep_frame=keep_frame,
            start_codon_mode=start_codon_mode,
            forward_anneal_length=forward_anneal_length,
            reverse_anneal_length=reverse_anneal_length,
        )

    return result


def _suggest_restriction_pairs(
    template: str, *, target_start: int, target_end: int,
    excluded: tuple[str, str], clamp: int, keep_frame: bool,
    start_codon_mode: str, forward_anneal_length: int | None,
    reverse_anneal_length: int | None, limit: int = 8,
) -> list[dict[str, str]]:
    """Find directional curated pairs that survive full PCR/digest simulation."""
    priority: list[tuple[str, str]] = []
    for pair in COMMON_ENZYME_PAIRS:
        left, right = (part.strip() for part in pair.split("/"))
        priority.append((left, right))
    priority.extend(
        (left, right)
        for left in RESTRICTION_ENZYMES
        for right in RESTRICTION_ENZYMES
        if left != right
    )

    suggestions: list[dict[str, str]] = []
    seen: set[tuple[str, str]] = set()
    for left, right in priority:
        pair = (left, right)
        if pair in seen or pair == excluded:
            continue
        seen.add(pair)
        if overhang(left) == overhang(right):
            continue
        try:
            candidate = design_pcr(
                template,
                target_start=target_start,
            ...
            continue
        if not candidate.is_clean or candidate.digest is None:
            continue
        suggestions.append({
            "left_enzyme": left,
            "right_enzyme": right,
            "left_overhang": candidate.digest.left_end.sequence,
            "right_overhang": candidate.digest.right_end.sequence,
        })
